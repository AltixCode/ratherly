...
describe("Paywall", () => {
  ...
  // The pill-tab carousel's navigation IS the tab row: one tab per surviving
  // benefit, kept in sync with the swipeable carousel below it. This pins the
  // count to the filtered list rather than a hardcoded four, same reasoning as
  // "renders no row for a claim this app does not make" above.
  it("renders one carousel tab per benefit this app actually claims", async () => {
    const { getAllByRole } = await renderWithProviders(<Paywall />);
    expect(getAllByRole("tab")).toHaveLength(4);
  });

  it("jumping to a benefit via its tab selects it, and swiping past the last tab has nothing further to select", async () => {
    const { getByLabelText } = await renderWithProviders(<Paywall />);
    const secondTab = getByLabelText(t("feat2Title"));
